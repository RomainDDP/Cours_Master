...
/* ***************************************************************** */
#include "RobotRemove.h"
#include "RTDS_messages.h"
#include "RTDS_gen.h"

#define RTDS_PROCESS_NUMBER RTDS_process_RobotRemove
#define RTDS_PROCESS_NAME RobotRemove
...
RTDS_TASK_ENTRY_POINT(RobotRemove)
  {
  ...
  #include "RobotRemove_tempVars.h"
  ...
  for ( ; ; )
    ...
    if ( RTDS_currentContext->currentMessage != NULL )
      ...
      switch(RTDS_currentContext->sdlState)
        {
        /* Transitions for state REMOVING */
        case RTDS_state_REMOVING:
          switch(RTDS_currentContext->currentMessage->messageNumber)
            {
            /* Transition for state REMOVING - message Sensor_RemoveDone */
            case RTDS_message_Sensor_RemoveDone:
              RTDS_MSG_QUEUE_SEND_TO_NAME(RTDS_message_Ack_RemoveDone, 0, NULL, "Supervisor", RTDS_process_Supervisor);
              RTDS_SDL_STATE_SET(RTDS_state_IDLE);
              break;
            /* Transition for state REMOVING - message Cmd_StopAll */
        ...
        case RTDS_state_IDLE:
          switch(RTDS_currentContext->currentMessage->messageNumber)
            {
            /* Transition for state IDLE - message Cmd_Remove */
            case RTDS_message_Cmd_Remove:
              RTDS_SDL_STATE_SET(RTDS_state_REMOVING);
              break;
