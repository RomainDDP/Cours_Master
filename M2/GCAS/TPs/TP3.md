...
* `pcst` pour définir une valeur constante,
* `pread`  pour un registre lu,
* `pwrite` pour un registre écrit.
...
```c++
Inst add("add %0, %1, %2", pwrite(6), pread(0), pread(1));
```

L'instruction `mov R3, #128` sera décrite par :

```c++
Inst movi("mov %0, #%1", pwrite(3), pcst(128));
```
...
> **NOTE** On pourra utiliser l'option `-print-select -stop-after-print` pour afficher le résultat de la sélection des instructions (même si certains quadruplets ne sont pas traduits).
