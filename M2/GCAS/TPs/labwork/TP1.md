...
* `E1 [E2 .. E3]` : accès à un champ de bit (bit poids fort _E2_ à bit de poids faible _E3_).
...
> **ATTENTION :** si BitFieldExpr avait une forme avec un seul indice, `\_hi` = `\_lo`, il sera bien que, après réduction, on garde cette propriété. Cela nous permettra d'améliorer les phases de compilation suivantes.
...
Vous pourrez tester la fonction avec les fichiers `test/eval.io` et `test/noconst.io`. Pour appeler le compilateur sur ces fichiers, on tapera la commande :
