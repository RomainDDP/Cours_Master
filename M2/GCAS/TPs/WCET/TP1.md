...
If you want to work on your laptop, you can use [this](http://www.tracesgroup.net/otawa/?page_id=419) to get a script in order to compile and install otawa (but it will take a while). This creates a directory named `otawa` and the commands are in directory `otawa/bin`. Notice that the directory `otawa` can be moved around without breaking the installation.

On our desktop, nothing needs to be done: OTAWA commands are already in the path.


**Assignment**

This document contains exercices with empty fields that you have to fill and then to deposit on the Moodle repository. Questions you have to answer are marked with symbol [=>]. When you have to write explanation, they have to be framed between [<<] and [>>].
...
`obviews` opens a window in your browser where all aspects of WCET computation are graphically displayed:
...
block (A) executed once more than block (B)?
	...
	[=>] Block (A) address: BB 17 (5c8)	    Execution count: 256
	
	[=>] Block (B) address:	BB 19 (540)		Execution count: 255
...
6. In the CFG view, the `main` function contains 2 calls to `icrc`. If you click on the two calling blocks, you get two different CFG. Observe the CFG call path at the top just above the left view, one is called from line 131 and the other one from line 134.
	...
	[<<]
        Those two were separated because different parameters where given, which changes the loop bounds for the function.
	[>>]

7. Record the WCET (it is called the initial WCET). Remove the file `crc.ff`.

	[=>] initial WCET = 223581 cycles
...
9. This creates a file in XML named `crc.ffx`. Open it with your preferred text editor and observe how the loop bounds are provided, taking into account the subprogram call chains leading to a particular loop. What are now the bound(s) of the loop at `crc.c:93`?
	...
	[=>] bound of loop at `crc.c:93` = min=0 and max=256
...
_oRange_ loop bound file?
	...
	[=>] new WCET = 115791 cycles
	
	[=>] Why is there a difference?
	
	[<<]
        First the given bounds for the loop are actually correct this time, and even the minimum is given.
	[>>]
...
`bubble` is a tiny application performing the sort of an integer array using the bubble sort algorithm.
...
7. What do you observe? Why?
	...
	[=>] The execution count is way higher than the one I calculate by hand.
